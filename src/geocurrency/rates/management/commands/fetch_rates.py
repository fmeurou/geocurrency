...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        """
        parser.add_argument("-f", '--from_date', type=str, help="Date in YYYY-MM-DD format."
                                                                   "Start fetching from this date, defaults to today")
        parser.add_argument("-t", '--to_date', type=str, help="Date in YYYY-MM-DD format."
    ...
    def handle(self, *args, **options):
        ...
        try:
            from_date = datetime.strptime(options.get('from_date', today), '%Y-%m-%d')
            to_date = datetime.strptime(options.get('to_date', today), '%Y-%m-%d')
        except ValueError:
            print("invalid dates")
            exit(-1)
